...
class ChannelCog(commands.Cog):
    ...
    async def channelstats(self, ctx):
        ...
        channel = ctx.channel
        embed = discord.Embed(title=f"Stats for {channel.name}", description = channel.category.name)
        embed.add_field(name = "Channel Guild", value=ctx.guild.name, inline=False)
        embed.add_field(name = "Channel ID", value = channel.id, inline = False)
        embed.add_field(name = "Channel Topic", value = channel.topic if channel.topic else "No Topic", inline = False)
